...
<script>
import { modifyPasswordSubmit } from '@/api/baseInfomation'
export default {
  name: 'Changepwd',
  data() {
  ...
  methods: {
    submitForm() {
      modifyPasswordSubmit({ 'oldpassword': this.ruleForm.oldPass, 'newpassword1': this.ruleForm.newPass, 'newpassword2': this.ruleForm.checkPass }).then(res => {
        if (res.re == -1) {
          this.$message({
            type: 'error',
            message: '旧密码不正确请重新输入'
